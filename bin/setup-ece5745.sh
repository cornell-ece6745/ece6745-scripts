...
# setup-ece5745.sh [-q]
#=========================================================================
# This setup script is for ECE 5745. It will set various environment
...
print ""
print " Running ECE 5745 setup script"
...
# file in the users home directory called .setup-ece5745 which has a
...
run_init_cmds="yes"
if [[ -f "${HOME}/.setup-ece5745" ]]; then
  run_init_cmds_last_ver=$(tail -1 "${HOME}/.setup-ece5745")
...
if [[ "${run_init_cmds}" == "yes" ]]; then
  ...
  echo "${run_init_cmds_curr_ver}" >> "${HOME}/.setup-ece5745"
fi

#-------------------------------------------------------------------------
# Determine which platform we are running on
#-------------------------------------------------------------------------
# The ECE 5745 setup script is only meant to be run on ecelinux, so we
...
if [[ ${server_class} == "unknown" ]]; then
  print ""
  print " The ECE 5745 tools are not supported on this machine."
...
# Setup global package installs for ECE 5745

print "  - Setting up global package install paths"

export STOW_PKGS_GLOBAL_ROOT="/classes/ece5745/install/stow-pkgs"
export STOW_PKGS_GLOBAL_PREFIX="${STOW_PKGS_GLOBAL_ROOT}/${ARCH}"

export BARE_PKGS_GLOBAL_ROOT="/classes/ece5745/install/bare-pkgs"
export BARE_PKGS_GLOBAL_PREFIX="${BARE_PKGS_GLOBAL_ROOT}/${ARCH}"

export RVM_PKGS_GLOBAL_ROOT="/classes/ece5745/install/rvm-pkgs"
...
if [[ ! -f "${HOME}/.ssh/ece5745-github.pub" ]]; then

  print "  - Setting up ssh keys for accessing github"

  netid=$(whoami)
  ssh-keygen -q -N "" -t rsa -C "${netid}@cornell.edu" -f "${HOME}/.ssh/ece5745-github"

  echo ""                                     >> "${HOME}/.ssh/config"
  echo "# added by setup-ece5745.sh"          >> "${HOME}/.ssh/config"
  echo "Host github.com"                      >> "${HOME}/.ssh/config"
  echo "  IdentityFile ~/.ssh/ece5745-github" >> "${HOME}/.ssh/config"
  echo ""                                     >> "${HOME}/.ssh/config"

fi

if ! grep -q "# key/X11 forwarding added by setup-ece5745.sh" "${HOME}/.ssh/config"; then

  print "  - Running ssh initialization to ensure key/X11 forwarding enabled"

  echo ""                          >> "${HOME}/.ssh/config"
  echo "# key/X11 forwarding added by setup-ece5745.sh" >> "${HOME}/.ssh/config"
  echo ""                          >> "${HOME}/.ssh/config"
...
chmod 600 ~/.ssh/ece5745-github.pub
chmod 600 ~/.ssh/ece5745-github
...
export VENV_PKGS_GLOBAL_ROOT="/classes/ece5745/install/venv-pkgs/${ARCH}"

ps_temp="$PS1"
source ${VENV_PKGS_GLOBAL_ROOT}/python2.7.13/bin/activate
PS1="${ps_temp}"
...
print "  - Setting up standard cells"

export STDCELLS_DIR="${BARE_PKGS_GLOBAL_ROOT}/noarch/saed-90nm-synopsys-cells"

function load_env_module
{
  print "  - Setting up $1"
  module load $1
}

load_env_module synopsys-vcs-vcsi-2016.6
load_env_module synopsys-lc-2016.12
load_env_module synopsys-dc-2016.12
load_env_module synopsys-icc-2016.12
load_env_module synopsys-pt-2016.12
...
if [[ "${run_init_cmds}" == "yes" ]]; then
  ...
  print "  - Setting up ICC presets"

  mkdir -p "$HOME/.synopsys_icc_presets/Layout"

  cp "${STOW_PKGS_GLOBAL_ROOT}/noarch/etc/synopsys-icc-presets/ece5745.tcl" \
     "$HOME/.synopsys_icc_presets/Layout"
